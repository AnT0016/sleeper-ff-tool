...
Python 3.11+, `src/` layout. **Phases 1–9 are DONE** — Phase 8 shipped the point-in-time data foundation (the lake + `build_training_frame`; its prelock cron captures the forward-only Sleeper projections **from the pre-season onward** — `season_type` stays `'pre'` until the regular season starts, so gating collection on it would risk losing Week 1's pre-lock values for good). **Phase 9 (in-house models over the lake) is DONE** — the weekly, K/DST and draft-value models are built, graded, and now a **selectable** projection source (default Sleeper) behind the live swap gate, which reads zero live weeks today so every surface stays on Sleeper until a model clears the forward bar from 2026 W1 (spec: [docs/plans/modeling.md](docs/plans/modeling.md), tickets #27–#34). See [docs/PROGRESS.md](docs/PROGRESS.md) for per-phase status. One-time setup: `python -m venv .venv` then `./.venv/Scripts/python -m pip install -e ".[dev]"`.
